...
const FormDialog = memo((props: IProps) => {
  ...
  const [, rolesError, , retryRoles] = useRetryableObservable(() => {
  ...
  return (
    ...
    >
      ...
      <form onSubmit={formik.handleSubmit}>
            ...
            <Fragment>
              <Grid container spacing={2}>
                <Grid item xs={12} sm={6}>
                  <TextField label='Valor' name='value' type='text' mask='money' formik={formik} />
                </Grid>
                <Grid item xs={12} sm={6}>
        ...
        <DialogActions>
          <Button onClick={props.onCancel}>{formik.values.id ? 'Fechar' : 'Cancelar'} Pedido</Button>
          {!formik.values.id && (
